...
namespace GestureControls
{
    public class KinectCursorManager
    {
        ...
        private CursorAdorner _cursorAdornder;
        ...
        private KinectCursorManager(Window window, KinectSensor sensor, FrameworkElement cursor)
        {
            ...
            if (KinectSensor.KinectSensors.Count > 0)
            {
                ...
                {
                    if (cursor == null)
                        _cursorAdornder = new CursorAdorner((FrameworkElement)window.Content);
                    else
                        _cursorAdornder = new CursorAdorner((FrameworkElement)window.Content, cursor);
        ...
        {
            _cursorAdornder.SetVisibility(true);
        ...
        {
            _cursorAdornder.SetVisibility(false);
        ...
        {
            UIElement element = GetElementAtScreenPoint(point, Application.Current.MainWindow);
            if (element != null)
            {
                element.RaiseEvent(new KinectCursorEventArgs(KinectInput.KinectCursorMoveEvent, point, z) { KinectCursor = _cursorAdornder });
                if (element != _lastElementOver)
                {
                    if (_lastElementOver != null)
                    {
                        _lastElementOver.RaiseEvent(new KinectCursorEventArgs(KinectInput.KinectCursorLeaveEvent, point, z) { KinectCursor = _cursorAdornder });
                    }

                    element.RaiseEvent(new KinectCursorEventArgs(KinectInput.KinectCursorEnterEvent, point, z) { KinectCursor = _cursorAdornder });
                }
            }
        ...
        private void HandleGestureTracking(float x, float y, float z)
        {
            ...
            if (Math.Abs(newPoint.Y - startPoint.Y) > _swipeDeviation)
            {
                ...
                if (SwipeOutOfBoundsDetected != null)
                    SwipeOutOfBoundsDetected(this, new KinectCursorEventArgs(point) { Z = z, KinectCursor = _cursorAdornder });
                ...
                (_swipeLength > 0 && newPoint.X - startPoint.X > _swipeLength))
            {
                ...
                if (SwipeDeteted != null)
                    SwipeDeteted(this, new KinectCursorEventArgs(point) { Z = z, KinectCursor = _cursorAdornder });
                return;
            }
